...
const splitFoods = (recipe) => {
  return recipe.ingredients.map(i => i.split(' ').slice(2).join(' '));
}
...
const stepActions = (recipe) => {
  return recipe.steps.map(s => s.split(' ').slice(0, 1).join(' '));
}
...
const totalSumCSV = (str) => {
  return Number(str.split(',').reduce((a, n) => Number(a) + Number(n)));
};
...
const extractVowels = (str) => {
  const noVowels = str.split('').filter(c => !/[aeiou]/gi.test(c)).join('');
  const vowels = str.split('').filter(c => /[aeiou]/gi.test(c)).sort().join('');
