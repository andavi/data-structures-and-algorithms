...
const reversedString = (arr) => {
  return arr.split('').reduce((a, c) => c + a, '');
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((a, n) => isPrime(n) ? ++a : a, 0);
}
...
const extractStat = (statName, arr) => {
  return arr.reduce((a, c) => {
    if (c.stat.name === statName) return c;
    return a;
  }, null);
};
...
const extractChildren = (arr) => {
  return arr.filter(c => /a/ig.test(c.name)).reduce((a, c) => {
    return c.children ? a.concat(c.children) : a;
  }, []);
};
